...
const TopMoversCard: React.FC<TopMoversCardProps> = ({ onSymbolClick }) => {
    ...
    const fetchTopMovers = async () => {
        ...
        try {
            // Direct fetch with 20-second timeout via Vite proxy
            const controller = new AbortController();
            const timeout = setTimeout(() => controller.abort(), 20000);

            const response = await fetch('/api/market/nifty100/top-movers', {
            ...
            const result: TopMoversData = await response.json();
