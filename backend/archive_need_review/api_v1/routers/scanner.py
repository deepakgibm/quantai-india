...
async def get_week52_breakouts(
    ...
    try:
        ...
        if not results:
            logger.info("week52-breakouts: Cache empty, running scanner with budget...")
            try:
                # Use a tighter budget for the API response to avoid 504
                await engine.run_scanner(timeout=15.0)
                results = await engine.get_cached_results()
            except Exception as scan_error:
                logger.error(f"week52-breakouts: Scanner failed: {scan_error}")
                return {
                    "status": "partial_success" if results else "pending",
                    "message": "The scan is taking longer than expected. Showing partial results." if results else "52-week breakout scan is in progress. Please try again.",
                    "high_breakouts": [],
                    "low_breakdowns": [],
                    "data": []
                }
...
async def get_momentum_status(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if engine is None:
            return {"status": "unavailable", "message": "Real-time scanner engine not initialized"}
