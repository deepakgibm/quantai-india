...
def insert_candles(conn, instrument_id, df, timeframe_minutes):
    """Bulk insert/upsert candles for a given timeframe"""
    ...
    try:
        ...
        # Upsert Logic for Refresh Daily
        cursor.execute(f"""
...
def resample_and_insert(conn, instrument_id, df_1m, target_tf_str, target_mins):
    ...
    try:
        ...
        if count > 0:
            print(f"  + {target_tf_str} ({target_mins}m): {count} candles (Resampled)")
            
    except Exception as e:
        print(f"    ! Resample Error ({target_tf_str}): {e}")

async def refresh_today_for_symbol(client, symbol: str):
    print(f"\nRefreshing {symbol} for TODAY...")
...
async def main():
    print("=" * 60)
    print(f"Starting TODAY Data Refresh ({datetime.now().date()})")
    print(f"Target: Daily + 3m/15m/1h (Resampled)")
    ...
    for i, symbol in enumerate(NIFTY_500_SYMBOLS):
        try:
            await refresh_today_for_symbol(client, symbol)
