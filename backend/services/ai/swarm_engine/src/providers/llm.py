...
class MockChatGoogleGenerativeAI:
    def __init__(self, model: str, temperature: float = 0.0, callbacks: Any = None):
        self.model = model
        self.temperature = temperature
        self.callbacks = callbacks
        self.tools = []

    def bind_tools(self, tools: Any) -> MockChatGoogleGenerativeAI:
        self.tools = tools
        return self

    def _get_mock_text(self, messages: Any) -> str:
        prompt_text = ""
        for m in messages:
        ...
        if "bull_advocate" in prompt_lower or "bull analyst" in prompt_lower or "bull advocate" in prompt_lower:
            return (
                "**Bull Analyst Report:**\n"
                "- **Technical Trend**: Trading above 20 and 50 Daily EMAs, indicating strong medium-term momentum.\n"
                "- **Momentum Indicators**: RSI is at 62 (bullish trend) and MACD shows a fresh bullish crossover.\n"
                "- **Price Action**: Volume expansion on upward candles confirms solid institutional buying interest near support.\n"
                "- **Conclusion**: Recommend BUY / LONG setup."
            )
        elif "bear_advocate" in prompt_lower or "bear analyst" in prompt_lower or "bear advocate" in prompt_lower:
            return (
                "**Bear Analyst Report:**\n"
                "- **Resistance Zone**: Immediate resistance is approaching, suggesting potential overhead supply.\n"
                "- **Volume Analysis**: Delivery selling volume near resistance ranges indicates active profit booking.\n"
                "- **Valuations**: High relative sector valuation signals potential near-term consolidation risk.\n"
                "- **Conclusion**: Wait for minor pullback before entry."
            )
        elif "risk_officer" in prompt_lower or "risk officer" in prompt_lower:
            return (
                "**Risk Officer Report:**\n"
                "- **Volatility**: ATR (Average True Range) remains within historical standard deviations.\n"
                "- **Exposure**: Recommend keeping the position size strictly limited to 2.0% of total capital.\n"
                "- **Safety**: Set strict stop-loss orders matching the recent swing support levels.\n"
                "- **Conclusion**: Risk parameters are fully satisfied. Pass."
            )
            ...
            try:
                ...
                return (
                    ...
                    "### Consensus Rationale\n"
                    "The Swarm Committee has completed the debate. Indicators could not be evaluated.\n\n"
                    "### Trade Details\n"
                    "- **Verdict**: HOLD\n"
                    "- **Target Price**: N/A\n"
                    "- **Stop Loss**: N/A\n"
        ...
        return "Technical indicators suggest stable consolidation. Recommend Hold."

    def invoke(self, messages: Any, config: Any = None) -> Any:
        from langchain_core.messages import AIMessage
        text = self._get_mock_text(messages)
...
def build_llm(*, model_name: Optional[str] = None, callbacks: Any = None) -> Any:
    """Construct a ChatGoogleGenerativeAI instance or Mock client in mock mode.

    Args:
        model_name: Model name; defaults to gemini-1.5-flash or gemini-2.0-flash.
        callbacks: Optional LangChain callbacks.

    Returns:
        ChatGoogleGenerativeAI or MockChatGoogleGenerativeAI instance.
    """
    from config import settings
    from langchain_google_genai import ChatGoogleGenerativeAI

    # Map name or use default
    name = model_name or os.getenv("LANGCHAIN_MODEL_NAME", "").strip() or "gemini-1.5-flash"
    
    # Handle aliases
    if "gpt" in name.lower() or "claude" in name.lower() or name == "default":
        name = "gemini-1.5-flash"

    temperature = float(os.getenv("LANGCHAIN_TEMPERATURE", "0.0"))
    
    # Return Mock client if mock responses or disabled AI features are configured
    if not settings.ENABLE_AI_FEATURES or settings.MOCK_AI_RESPONSES:
        logger.info("Initializing MockChatGoogleGenerativeAI for offline/mock mode")
        return MockChatGoogleGenerativeAI(model=name, temperature=temperature, callbacks=callbacks)

    logger.info("Initializing ChatGoogleGenerativeAI with model: %s", name)
