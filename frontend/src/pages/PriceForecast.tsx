...
const PriceForecast: React.FC = () => {
    ...
    const renderChart = () => {
        ...
        const timestamps = [...hist, ...fore].map(c => {
            try {
                return new Date(c.timestamp).toLocaleString('en-IN', {
        ...
        // Historical close prices (nulls for forecast region)
        const historicalData = [...hist.map(c => c.close ?? null), ...fore.map(() => null)];

        // Forecast line: last historical point bridges into forecast
        // IMPORTANT: Keep array length equal to timestamps length (no extra bridge point)
        const forecastData_ = hist.map((_c, i) =>
            i === hist.length - 1 ? hist[hist.length - 1].close : null
        );
        const forecastLine = [...forecastData_, ...fore.map(c => c.close ?? null)];

        // Confidence bands (no stacking — use separate lines)
        const upperBand = [...hist.map(() => null), ...fore.map(c => c.upper ?? null)];
        const lowerBand = [...hist.map(() => null), ...fore.map(c => c.lower ?? null)];
        ...
        const option: echarts.EChartsOption = {
            ...
            legend: {
                data: ['Historical', 'Forecast', 'Upper Band', 'Lower Band'],
            ...
            series: [
                ...
                },
                {
                    ...
                    data: upperBand,
                    symbol: 'none',
                    lineStyle: { opacity: 0.3, color: '#1c7ed6', width: 1, type: 'dotted' },
                    areaStyle: { color: 'rgba(28,126,214,0.08)' }
                },
                {
                    name: 'Lower Band',
                    type: 'line',
                ...
                ...(hist.length > 0 ? [{
                    ...
                    markLine: {
                        silent: true,
                        symbol: 'none',
                        label: { show: true, position: 'end' as const, formatter: 'Forecast Boundary', color: '#1c7ed6', fontSize: 10 },
                        lineStyle: { color: '#1c7ed6', width: 1, type: 'solid' as const, opacity: 0.5 },
